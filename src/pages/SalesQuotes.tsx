...
export default function SalesQuotes() {
  ...
  useEffect(() => {
    const fetchQuotes = async () => {
      try {
        ...
        const quotesData = Array.isArray(response.data) ? response.data : (response.data?.value || []);
  ...
  const filteredQuotes = useMemo(() => {
    return quotes.filter(
      (quote) =>
        (quote.Document_No_?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        quote.Sell_to_Customer_No_?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600 dark:text-gray-400">Loading sales quotes...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-lg">
          {filteredQuotes.length > 0 ? (
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    {currentQuotes.map((quote) => (
                      <tr key={quote.Document_No_ || quote.No || quote.QuoteNo} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                          {quote.Document_No_ || quote.No || quote.QuoteNo || 'N/A'}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                          <div className="font-medium">{quote.Sell_to_Customer_Name}</div>
                          <div className="text-xs text-gray-400">{quote.Sell_to_Customer_No_}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300 text-right">
                          {formatCurrency(quote.Amout_Including_VAT || 0)}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <Link
                            to={`/sales/quotes/${quote.Document_No_ || quote.No || quote.QuoteNo}`}
            ...
            <div className="text-center py-12">
              ...
              <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-white">No sales quotes found</h3>
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {searchTerm ? 'Try adjusting your search or filter to find what you\'re looking for.' : 'Get started by creating a new sales quote.'}
              </p>
              <div className="mt-6">
                <button
                  type="button"
